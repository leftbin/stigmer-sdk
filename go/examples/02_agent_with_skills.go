...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to create inline skill: %v", err)
	}

	// Example 2: Create agent with inline skill
	agentWithInline, err := agent.New(
	...
	if err != nil {
		log.Fatalf("Failed to create agent with inline skill: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to create org agent: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to create mixed agent: %v", err)
	}
