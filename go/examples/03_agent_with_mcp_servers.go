...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to create API MCP server: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to create custom Docker MCP server: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to create AWS MCP server: %v", err)
	}
